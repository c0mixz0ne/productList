...
<template>
  <draggable ghost-class="ghost-device" class="device-list" :class="editable ? 'editable' : ''" id="devices" group="devices" :list="devices" :disabled="!editable" item-key="index" tag="ul">
    <template #item="{ element: device, index: indexDevice }">
      <li :class="editable ? 'move' : ''" class="device">
        ...
        <ul v-if="device.nodes.length || editable" class="node-list">
          <draggable ghost-class="ghost-node" :class="editable && device.nodes.length ? 'margin-bottom' : ''" class="nodes" id="nodes" group="nodes" :list="device.nodes" :disabled="!editable" item-key="index" tag="ul">
            <template #item="{ element: node, index: indexNode }">
              <li class="node">
                <input :id="String(indexNode)" :disabled="!editable" type="text" v-model="node.name" />
                <button v-if="editable" @click="deleteNode(indexDevice, indexNode)">
                  Удалить узел
                </button>
              </li>
            </template>
